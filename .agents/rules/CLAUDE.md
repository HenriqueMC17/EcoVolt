...
Esta diretriz estabelece as convenções de engenharia lógica, design engineering e viewport para agentes baseados em lógica e visão computacional (como a família **Claude**) operando no repositório do **EcoVolt**.
...
* **TypeScript & Tipagem Rígida:** NUNCA utilize o tipo genérico `any` para novas implementações. Todas as assinaturas de funções, estados e APIs devem ser explicitamente tipadas com interfaces ou types estritos.
...
* **Gestão de Estado Reativa:** Mantenha a renderização em 60fps estável e minimize re-renders desnecessários usando hooks de estado adequados.

---

## 🧪 3. Protocolo de QA e Cobertura de Testes

1. **Testes Unitários e E2E:** Qualquer funcionalidade crítica ou novos módulos que realizem cálculos ou alterem fluxos de dados devem ser verificados por testes locais ou E2E (via Playwright no apps/landing-page).
2. **Assertion de Cobertura:** A cobertura mínima aceitável para qualquer alteração lógica de produção é de **80%**.
