# 🤖 DIRETRIZ OPERACIONAL DO GEMINI (`GEMINI.md`)

Esta diretriz estabelece regras específicas de execução e engenharia de contexto para modelos da família **Google Gemini** operando no repositório do **EcoVolt**.
...
1. **Divulgação Progressiva:** Ao referenciar diretórios e arquivos, use caminhos absolutos ou links markdown completos com o esquema `file:///`.
2. **Preservação de Contexto Histórico:** Ao iniciar uma nova sessão ou interagir com o usuário, leia primeiro as KIs (Knowledge Items) e as logs de conversas passadas para reter as diretivas e decisões estratégicas tomadas anteriormente.
...
* **Controle de Quebras de Linha (LF vs CRLF):** O Windows utiliza `\r\n` por padrão. Ao realizar operações com `replace_file_content` ou `multi_replace_file_content`, extraia primeiro o conteúdo original do arquivo para garantir correspondência exata de caracteres.
...
2. **Visual Context (Stitch & Figma):** Ao realizar alterações visuais orientadas por design ou receber feedback de layouts, compare a árvore de componentes estáticos antes de disparar re-gerações de telas completas.

---

## 🚨 4. Protocolo de Resolução de Erros de Terminal

Ao executar comandos via PowerShell e receber logs de erro, siga o fluxograma de resolução autônoma corrigindo o estado subjacente do workspace antes de tentar nova execução.

*   **Evitar Loops de Comando:** Não execute repetidamente os mesmos comandos de terminal se estes estiverem falhando com o mesmo status code.
