...
export const Problem = () => {
  ...
  return (
    <Section id="problema" className="bg-gradient-to-b from-slate-50 dark:from-slate-950 to-slate-950 pt-16 pb-32 md:pt-24 md:pb-48 overflow-hidden relative">
          ...
          <div className="space-y-6">
            {problems.map((item, i) => (
              <GlassCard 
                key={i} 
                variant="dark" 
                className="group flex flex-col md:flex-row gap-6 items-start md:items-center p-8 border-white/5 bg-slate-900/40 hover:bg-slate-900/80 hover:-translate-y-1 transition-all shadow-[0_0_40px_-10px_rgba(0,0,0,0.3)] hover:shadow-[0_0_60px_-10px_rgba(0,0,0,0.5)]"
              >
                <div className={`w-14 h-14 shrink-0 rounded-2xl ${item.bg} flex items-center justify-center ${item.accent} shadow-xl shadow-black/20 group-hover:scale-110 transition-transform`}>
                  <item.icon size={28} aria-hidden="true" />
                </div>
                <div className="space-y-2">
                  <h3 className="text-white text-xl md:text-2xl font-bold tracking-tight">{item.title}</h3>
                  <Text className="text-slate-400 font-medium leading-snug">{item.description}</Text>
                </div>
              </GlassCard>
            ))}
          </div>
