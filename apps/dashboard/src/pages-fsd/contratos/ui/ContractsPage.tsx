    ...
    <div className="space-y-10 pb-20 animate-luxury">
      <header className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
        <div className="space-y-4">
          ...
          <Typography className="text-text-muted max-w-2xl">
            Centralize o controle jurídico de propostas, SLAs e termos de responsabilidade técnica de todas as operações.
          </Typography>
        </div>
        
        <Button className="h-14 px-10 rounded-full font-black uppercase tracking-widest text-[10px]">
      ...
      {/* Filter Bar */}
      <div className="flex flex-col md:flex-row gap-5 items-center bg-black/40 backdrop-blur-md border border-white/10 p-3 rounded-full">
        ...
        <div className="flex gap-4 w-full md:w-auto">
           <button className="h-14 px-8 rounded-full bg-white/5 border border-white/10 text-white/60 flex items-center gap-3 hover:text-white hover:bg-white/10 transition-all uppercase tracking-widest text-[10px] font-black">
             <Filter size={16} />
             Filtros
           </button>
        </div>
        ...
        {mockContracts.map((contract, idx) => {
          ...
          return (
            <motion.div 
              ...
              className="glass-card group p-8 hover:border-primary/30 transition-all duration-500 cursor-pointer"
            >
              <div className="flex flex-col xl:flex-row justify-between gap-10">
                <div className="flex gap-6 flex-1">
                  <div className="w-16 h-16 rounded-2xl bg-black/40 border border-white/10 flex items-center justify-center text-white/40 group-hover:text-primary group-hover:border-primary/30 transition-all duration-500">
                    <FileText size={28} />
                  </div>
                  <div className="space-y-1">
                    <div className="flex items-center gap-3">
                      <Typography className="text-[10px] font-black tracking-widest text-primary uppercase">{contract.id}</Typography>
                    ...
                    <div className="flex flex-wrap items-center gap-x-6 gap-y-2 pt-2">
                      <div className="flex items-center gap-2 text-white/60">
                        <UserCheck size={14} />
                        <span className="text-xs font-medium">Fornecedor: <span className="text-white/80">{contract.provider}</span></span>
                      </div>
                      <div className="flex items-center gap-2 text-white/60">
                        <History size={14} />
                        <span className="text-xs font-medium">Vigência até: <span className="text-white/80">{new Date(contract.expiry).toLocaleDateString('pt-BR')}</span></span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row items-center gap-8 xl:min-w-[400px]">
                  <div className="flex-1 text-right md:text-left xl:text-right">
                    <Typography className="text-white/40 text-[10px] font-black uppercase tracking-widest">Valor do Contrato</Typography>
                    <Typography className="text-2xl font-bold text-white">R$ {contract.value.toLocaleString('pt-BR')}</Typography>
                  </div>
                  
                  <div className="flex gap-3">
                    <button className="p-4 rounded-2xl bg-black/40 border border-white/10 text-white/40 hover:text-white hover:border-white/20 transition-all">
                      <Eye size={20} />
                    </button>
                    <button className="p-4 rounded-2xl bg-black/40 border border-white/10 text-white/40 hover:text-white hover:border-white/20 transition-all">
                      <Download size={20} />
                    ...
                    <Button 
                      ...
                      variant="outline"
                      className="h-14 px-6 rounded-2xl bg-primary/10 border border-primary/20 text-primary hover:bg-primary/20 hover:border-primary/40 transition-all font-black text-[10px] uppercase tracking-widest"
                    >
                      Ver Dossier
