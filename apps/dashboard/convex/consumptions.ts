...
export const updateConsumption = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.predictedKwh !== undefined) updates.predictedKwh = args.predictedKwh;
    if (args.actualKwh !== undefined) updates.actualKwh = args.actualKwh;
...
export const removeConsumption = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await logActivityHelper(ctx, {
      ...
      entityType: "consumptions",
      details: {
        summary: `Registro de consumo removido para o evento ${oldConsumption?.eventId}.`,
      },
    });
  },
});
