...
export const removeFinancial = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await logActivityHelper(ctx, {
      ...
      entityType: "financials",
      details: {
        summary: `Transação financeira de R$ ${oldFinancial?.amount.toFixed(2)} (${oldFinancial?.category}) removida.`,
      },
    });
  },
});
...
export const getFinancialStats = query({
  ...
  handler: async (ctx, args) => {
    ...
    let transactions: Doc<"financials">[] = [];

    if (user.role === "admin" || user.role === "operator") {
      transactions = await ctx.db.query("financials").collect();
    } else if (user.role === "event_company") {
      const companyId = user.companyId;
      if (companyId) {
        const events = await ctx.db
